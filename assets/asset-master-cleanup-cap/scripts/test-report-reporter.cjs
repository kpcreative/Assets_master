...
class TestReportReporter {
  constructor(globalConfig, options = {}) {
    ...
    this._outputPath = options.outputPath ? join(this._rootDir, options.outputPath) : join(this._rootDir, "test_report.json");
  ...
  onRunComplete(_contexts, results) {
    ...
    for (const file of results.testResults) {
      for (const t of file.testResults) {
        tests.push({ name: t.fullName || t.title, outcome: t.status === "passed" ? "passed" : t.status === "failed" ? "failed" : "skipped", duration: round((t.duration || 0) / 1000, 4) });
      }
    }
    ...
    const section = { name: this._sectionName, marker: this._sectionMarker, total, passed, failed, skipped, score, tests };
...
function readCoverage(coverageDir) {
  ...
  let data;
  try { data = JSON.parse(readFileSync(finalPath, "utf8")); } catch { return null; }
  let total = 0, covered = 0;
  for (const file of Object.values(data)) {
    const s = file?.s ?? {};
    for (const v of Object.values(s)) { total += 1; if (v > 0) covered += 1; }
  }
  if (!total) return null;
  return round((covered / total) * 100, 2);
}
function round(n, digits) { const f = 10 ** digits; return Math.round(n * f) / f; }
